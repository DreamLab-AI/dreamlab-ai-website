...
|---|---|---|---|---|
| `dreamlab-ai.com` (+ mirror `thedreamlab.uk`) | `49e70de557b31d4c6d98fc1f3d0d15ce2623a4dc` | `main` at 411352b (all four kit crates `nostr-bbs-core`/`config`/`mesh`/`rate-limit` 1.0.0-beta.11 on crates.io; augmentation-conditions governance — task-property triple, effective tier, application receipts, reviewer telemetry, HMAC-keyed calibration sampling which REQUIRES the relay secret `CALIBRATION_SELECTION_KEY`; user-feedback fixes — DM history, search, mobile thread layout, PWA icons, custom emoji, mentions, unread ingress; at c076992: search by text via the relay (NIP-50) rather than the admin-fed vector index, a who-reacted popover per emoji, relay 31403 admission build fix, rustls 0.23.45 for RUSTSEC-2026-0285, colloquy knowledge kinds 38410-38412.; at 411352b: ontology governance — Promote activated, Demote added, pending proposals expire via the relay's five-minute cron sweep; D1 migration 0007 (additive, nullable `broker_cases.stale_after` + partial index) applied at relay start-up by `ensure_schema()`; at aabfb85: member wallets on sidestr:dreamlab (testnet4) and DREAM tips, gated on `window.__ENV__.SIDESTR_WALLET` (ADR-2015; client-only, no worker or D1 change), and the admin Pending tab reads every whitelist page so approved members leave Pending (ADR-2014 phase 0); at 21562c6: members signed in with a browser signer spend through `window.nostr.sidestr.signTransaction` (sidestr spec proposals/browser-signer.md; reference signer Podkey 0.0.9) and transaction events come from a throwaway key (ADR-2015 D8; sidestr-wallet 0.4.1; client-only); at d79ae26: the review and tip hints say when Podkey (0.0.10, sidechain spends opt-in) will first ask to turn spends on, and an `unsupported` refusal says how (client-only); at c3f147f: sidestr-core 0.3.2 (builds without std; no behaviour change); at 87c580c: user cards show viewer-scoped activity via relay `GET /api/profile-stats`, replaced keys (`pubkey_aliases`) fold into their successor via public `GET /api/profiles/successors` and drop out of profile search, topics open at the newest post, search results open at the message via `/go/:event_id` (relay + client; no D1 migration, no secret); at 3a7f329: end-to-end encrypted zones (ADR-2016) — zone-key epochs granted by NIP-59 gift wrap (rumor kind 21453), kind-42 content NIP-44 encrypted to the zone key with a `zk` tag, relay refuses plaintext into encrypted bound channels; all dormant unless the relay var and `window.__ENV__.ENCRYPTION_ENABLED` are exactly `"true"`; public zones never enforced; per-zone `agent_keys` (no D1 migration, no secret); at 49e70de: zone keys persist and a grant is never lost to a transient failure, idle members keep receiving live events after the relay wakes, relay CORS answers each allowed origin (not only the first), and granting a zone key can also send earlier epochs so new members read history (relay + client; no D1 migration, no secret)) | `integrated` | ✔ |

<!-- pin-check:canonical-kit-sha -->
```
CANONICAL_KIT_SHA=49e70de557b31d4c6d98fc1f3d0d15ce2623a4dc
...
| `7dfe8a6` | `soak-fix-sprint-2026-07` | Superseded. @mention roster mode. The mention dropdown's three sources were all empty at query-open except the hardcoded 4-bot seed: the relay profile search 400'd on <2 chars (client gated at 2 too) and the ProfileCache is cold on a fresh load, so a bare `@` showed only the "house agents". Relay `/api/profiles/search` now serves a recent-members roster on an empty `q` (`ORDER BY last_kind0_at DESC LIMIT` — bounded + indexed, not a wildcard scan) and admits 1-char filter queries; client `NETWORK_SEARCH_MIN_LEN` 2→0 and `search_profiles` no longer short-circuits on empty, so a bare `@` fetches the roster (debounced). Real members lead; seed bots remain as offline fallback. Relay-worker + client. |
| `335bc3f` | `soak-fix-sprint-2026-07` | Superseded. Auto-redeem zone-bound invites on landing. A `/join/<code>` invite granted its zone cohort only when a returning-from-signup user manually clicked "Join <zone>"; the invite code is never threaded into the signup/registration flow, so a user who didn't return to the join page (or closed the tab after signup) kept only `["members"]` — the reported "minimoonoir/dreamlab invite link doesn't assign the zone" bug. `JoinPage` now fires the redeem automatically the moment an authenticated visitor views a valid, zone-bound invite (the returnTo-from-signup landing, or an already-registered user re-clicking the link); one-shot guarded, idempotent server-side, Join button retained as a manual/retry affordance. Client-only. |
