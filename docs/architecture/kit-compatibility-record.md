...
|---|---|---|---|---|
| `dreamlab-ai.com` (+ mirror `thedreamlab.uk`) | `7dfe8a69dd1034cac5e3892a1ac438da51b94408` | `soak-fix-sprint-2026-07` (@mention roster mode — a bare `@` now shows real members, not just the house-agent seed) | `integrated` | ✔ |

<!-- pin-check:canonical-kit-sha -->
```
CANONICAL_KIT_SHA=7dfe8a69dd1034cac5e3892a1ac438da51b94408
...
|---|---|---|
| `7dfe8a6` | `soak-fix-sprint-2026-07` | Current (canonical — matches `CANONICAL_KIT_SHA` above and the `KIT_REF` pins). @mention roster mode. The mention dropdown's three sources were all empty at query-open except the hardcoded 4-bot seed: the relay profile search 400'd on <2 chars (client gated at 2 too) and the ProfileCache is cold on a fresh load, so a bare `@` showed only the "house agents". Relay `/api/profiles/search` now serves a recent-members roster on an empty `q` (`ORDER BY last_kind0_at DESC LIMIT` — bounded + indexed, not a wildcard scan) and admits 1-char filter queries; client `NETWORK_SEARCH_MIN_LEN` 2→0 and `search_profiles` no longer short-circuits on empty, so a bare `@` fetches the roster (debounced). Real members lead; seed bots remain as offline fallback. Relay-worker + client. |
