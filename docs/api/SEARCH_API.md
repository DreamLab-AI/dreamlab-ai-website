...
Vector similarity search over R2-stored `.rvf` indices. The RVF store is compiled directly into the worker binary (`store.rs` `VectorStore` — no separate WASM module or JS-WASM bridge). Embeddings are 384-dimensional, generated by Cloudflare Workers AI `@cf/baai/bge-small-en-v1.5` with a deterministic hash fallback (`hash-fallback-v1`) when the AI binding is absent. Source: `nostr-rust-forum/crates/nostr-bbs-search-worker/` (the legacy `community-forum-rs` tree is superseded by the kit).

**Base URL:** `https://dreamlab-search-api.solitary-paper-764d.workers.dev` (live). The branded `https://search.dreamlab-ai.com` is the documented end-state but is not provisioned in DNS.
...
> **Superseded design.** The standalone `rvf_wasm` ES-module microkernel below was the legacy `community-forum-rs` implementation. In the deployed kit (`nostr-bbs-search-worker`), the RVF store is an in-crate Rust module (`store.rs`: `VectorStore::{insert, delete, search, to_rvf_bytes, from_rvf_bytes}`) compiled into the worker binary, and embeddings come from the Workers AI binding. The R2/KV storage layout and request flow remain as diagrammed.
...
The store and id-label mappings persist across requests within the same Worker isolate.
...
- `embedding`: 384-dimensional float array — OR `query`: a text string, embedded server-side with the same model used at ingest time (one of the two is required)
...
Generate embeddings from text. Primary path is Cloudflare Workers AI `@cf/baai/bge-small-en-v1.5` (reported as `"model": "bge-small-en-v1.5"`); a deterministic hash fallback (`"hash-fallback-v1"`) is used only when the AI binding is absent.
...
> **Superseded.** These exported function names belonged to the legacy standalone `rvf_wasm` module. The deployed kit implements the same operations as in-crate `VectorStore` methods: `new`, `from_rvf_bytes` (open), `insert` (ingest), `search` (query), `delete`, `count`, `to_rvf_bytes` (export).
...
| `AI` | Workers AI | `@cf/baai/bge-small-en-v1.5` semantic embeddings (384-dim) |
| `REPLAY_DB` | D1Database | `dreamlab-auth` -- shared NIP-98 replay table |
| `ALLOWED_ORIGIN` / `ALLOWED_ORIGINS` | [vars] | CORS origins |
| `RVF_STORE_KEY` | [vars] | R2 key (`dreamlab.rvf`) |
| `ADMIN_PUBKEYS` | [vars] | Static admin set for admin-gated search ops |

Source: `forum-config/deploy/search-worker.wrangler.toml`.
