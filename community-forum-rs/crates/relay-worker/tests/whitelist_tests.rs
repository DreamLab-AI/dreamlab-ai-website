...
// ---------------------------------------------------------------------------
//
// The legacy cohort taxonomy maps to three booleans: home, dreamlab,
// minimoonoir. Admins always have all three. cross-access cohort grants all.
...
fn compute_access_flags(is_admin: bool, cohorts: &[&str]) -> (bool, bool, bool) {
    ...
    (has_home, has_dreamlab, has_minimoonoir)
}

#[test]
fn admin_has_all_three_zones() {
    let (h, d, m) = compute_access_flags(true, &[]);
    assert!(h && d && m, "admin should have access to all zones");
}
...
fn lobby_user_has_only_home_access() {
    let (h, d, m) = compute_access_flags(false, &["lobby"]);
    assert!(h);
    assert!(!d);
    assert!(!m);
}
...
#[test]
fn dreamlab_business_has_only_dreamlab_access() {
    let (h, d, m) = compute_access_flags(false, &["business"]);
    assert!(!h);
    assert!(d);
    assert!(!m);
}
...
fn cross_access_grants_all_three_zones() {
    let (h, d, m) = compute_access_flags(false, &["cross-access"]);
    assert!(h);
    assert!(d);
    assert!(m);
}
...
fn unknown_cohort_grants_nothing() {
    let (h, d, m) = compute_access_flags(false, &["mystery-cohort"]);
    assert!(!h);
    assert!(!d);
    assert!(!m);
}
...
fn empty_cohorts_grants_nothing() {
    let (h, d, m) = compute_access_flags(false, &[]);
    assert!(!h);
    assert!(!d);
    assert!(!m);
}
...
#[test]
fn trainers_cohort_grants_dreamlab() {
    let (_, d, _) = compute_access_flags(false, &["trainers"]);
    assert!(d);
}

#[test]
fn trainees_cohort_grants_dreamlab() {
    let (_, d, _) = compute_access_flags(false, &["trainees"]);
    assert!(d);
}

#[test]
fn ai_agents_cohort_grants_dreamlab() {
    let (_, d, _) = compute_access_flags(false, &["ai-agents"]);
    assert!(d);
}

#[test]
fn agent_cohort_grants_dreamlab() {
    let (_, d, _) = compute_access_flags(false, &["agent"]);
    assert!(d);
}

#[test]
fn visionflow_full_cohort_grants_dreamlab() {
    let (_, d, _) = compute_access_flags(false, &["visionflow-full"]);
    assert!(d);
}

#[test]
fn business_only_cohort_grants_dreamlab() {
    let (_, d, _) = compute_access_flags(false, &["business-only"]);
    assert!(d);
}

#[test]
fn minimoonoir_only_grants_minimoonoir() {
    let (_, _, m) = compute_access_flags(false, &["minimoonoir-only"]);
    assert!(m);
}

#[test]
fn minimoonoir_business_grants_minimoonoir() {
    let (_, _, m) = compute_access_flags(false, &["minimoonoir-business"]);
    assert!(m);
}

#[test]
fn multi_cohort_union_grants_all_implied() {
    let (h, d, m) = compute_access_flags(false, &["lobby", "trainers", "minimoonoir"]);
    assert!(h);
    assert!(d);
    assert!(m);
}
