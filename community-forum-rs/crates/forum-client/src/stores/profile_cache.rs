...
///
/// `alice@dreamlab-ai.com` -> `@alice` for our own domain.
...
pub fn format_nip05_handle(nip05: &str) -> String {
    ...
    if let Some((local, domain)) = trimmed.split_once('@') {
        if domain.eq_ignore_ascii_case("dreamlab-ai.com")
            || domain.eq_ignore_ascii_case("pods.dreamlab-ai.com")
        {
            return format!("@{}", local);
        }
...
mod tests {
    ...
    fn nip05_local_domain_is_short() {
        assert_eq!(format_nip05_handle("alice@dreamlab-ai.com"), "@alice");
        assert_eq!(format_nip05_handle("bob@pods.dreamlab-ai.com"), "@bob");
    }

    #[test]
    fn nip05_external_domain_is_long() {
        assert_eq!(
            format_nip05_handle("alice@example.com"),
            "@alice@example.com"
        );
    }
    ...
    fn parse_wrapped_response() {
        ...
        assert_eq!(parsed[0].nip05.as_deref(), Some("alice@dreamlab-ai.com"));
    }
    ...
    fn entry_best_label_precedence() {
        let mut e = ProfileEntry {
            ...
            picture: None,
            nip05: Some("alice@dreamlab-ai.com".into()),
            fetched_at: 1,
        };
