...
//! first visit. The `dreamlab:welcomed` flag in localStorage prevents
...
///
/// Checks `localStorage` for the `dreamlab:welcomed` key. If absent, shows
...
pub fn WelcomeModal() -> impl IntoView {
    ...
    Effect::new(move |_| {
        let should_show = web_sys::window()
            ...
            .map(|s| s.get_item("dreamlab:welcomed").ok().flatten().is_none())
            .unwrap_or(false);
