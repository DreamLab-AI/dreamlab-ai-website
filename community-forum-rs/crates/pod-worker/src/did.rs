...
//! for WASM Workers (no tokio dependency). Logic is sourced from DreamLab's
...
/// - `https://pods.dreamlab-ai.com/<hex>/...` — hex in path must match.
pub fn verify_webid_tag(webid_uri: &str, event_pubkey: &str) -> bool {
    ...
    // Pod WebID: https://pods.dreamlab-ai.com/{pubkey}/profile/card#me
    if let Some(path) = webid_uri.strip_prefix("https://pods.dreamlab-ai.com/") {
...
mod tests {
    ...
    fn tier3_carries_webid_and_relay() {
        let pk = NostrPubkey::from_hex(PK_HEX).unwrap();
        let webid = "https://pods.dreamlab-ai.com/0000.../profile/card#me";
        let pod = "https://pods.dreamlab-ai.com/0000.../";
        let relay = "wss://relay.dreamlab-ai.com";
    ...
    fn verify_webid_tag_pod_url() {
        let pk = "a".repeat(64);
        let uri = format!("https://pods.dreamlab-ai.com/{pk}/profile/card#me");
