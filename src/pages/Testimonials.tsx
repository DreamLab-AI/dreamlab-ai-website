          ...
          {testimonials.map((testimonial, index) => (
            ...
            >
              <CardContent className="p-8 relative">
                ...
                {/* Quote text */}
                <blockquote className="text-foreground/90 leading-relaxed mb-6 pr-10 text-lg italic">
                  "{testimonial.quote}"
                </blockquote>
...
const TestimonialsMobile = () => {
  // Featured first; stable order preserved for the rest.
  const ordered = [...(testimonials as Testimonial[])].sort(
    (a, b) => (b.featured ? 1 : 0) - (a.featured ? 1 : 0)
  );

  return (
    <>
      <section id="main-content" className="pt-11 px-6 pb-1" aria-label="Impact stories">
        <h1 className="text-m-h1s text-[#FAFAFA] [text-wrap:pretty]">
          What people left with.
        </h1>
      </section>

      <section className="px-6 pb-11" aria-label="Testimonials">
        {ordered.map((t) => {
          const context = [t.programme, t.location_detail]
            .filter(Boolean)
            .join(" · ")
            .toUpperCase();
          return (
            <div key={t.id} className="border-t border-dlm-hairline py-[26px]">
              <blockquote className="text-m-quote text-[#FAFAFA] [text-wrap:pretty]">
                {`“${t.quote}”`}
              </blockquote>
              ...
              {context && (
                <p className="text-[11px] font-mono tracking-[0.12em] text-white/40 mt-1.5">
                  {context}
                </p>
              )}
            </div>
