...
describe("AIChatFab", () => {
  ...
  it("renders the agent reply and re-enables input when a reply arrives", async () => {
    const Fab = await loadFab({ relay: RELAY, jarvis: JARVIS });
    render(<Fab />);
    openPanel();
    ...
    await waitFor(() => expect(h.state.onReply).not.toBeNull());
    await waitFor(() => expect(h.sendQuestionMock).toHaveBeenCalled());
    ...
    expect(
      await screen.findByText(/residential AI training cohorts/i)
    ).toBeInTheDocument();
    await waitFor(() => expect(getInput()).not.toBeDisabled());
  });

  it("shows a delivery-failure message and re-enables input when the send is rejected", async () => {
    const Fab = await loadFab({ relay: RELAY, jarvis: JARVIS });
    ...
    expect(
      await screen.findByText(/message couldn't be delivered/i)
    ).toBeInTheDocument();
    await waitFor(() => expect(getInput()).not.toBeDisabled());
  });
