...
// =============================================================================
// SECURITY: URL Deny-list Configuration (SSRF Protection)
// =============================================================================

/**
 * Deny-list approach: Block known dangerous domains instead of maintaining
 * a fragile allow-list. This allows previews for any legitimate public site
 * while blocking known malicious or problematic domains.
 *
 * Combined with private IP blocking (below), this provides SSRF protection
 * without breaking previews for valid but unlisted domains.
 */
const BLOCKED_DOMAINS = new Set([
	...
	// Known malware/phishing domains (examples - extend as needed)
	'bit.ly', // URL shorteners often used for phishing - consider allowing with caution
]);
...
/**
 * Check if a hostname or IP is a private/internal address (SSRF protection)
 */
function isPrivateHost(hostname: string): boolean {
	...
	// Check blocked IP patterns (private ranges, localhost, metadata endpoints)
 ...
 * Uses deny-list approach: block known dangerous patterns, allow everything else
...
function validateUrlSecurity(urlString: string): string | null {
	...
	// 1. Validate scheme - HTTPS only (HTTP is insecure)
	if (parsed.protocol !== 'https:') {
		return 'Only HTTPS URLs are allowed';
	}

	// 2. Block private/internal addresses (SSRF protection)
	if (isPrivateHost(parsed.hostname)) {
		return 'Access to internal/private addresses is not allowed';
	}

	// 3. Check domain deny-list (known malicious/problematic domains)
	if (isDomainBlocked(parsed.hostname)) {
		return `Domain "${parsed.hostname}" is blocked`;
	}

	// 4. Block URLs with credentials (prevent credential leakage)
	if (parsed.username || parsed.password) {
		return 'URLs with credentials are not allowed';
	}

	// 5. Block non-standard ports (only allow 443 for HTTPS)
	// This prevents port scanning via SSRF
	if (parsed.port && parsed.port !== '443') {
		return 'Non-standard ports are not allowed';
	}

	// 6. Validate hostname has a TLD (basic sanity check)
	if (!parsed.hostname.includes('.') && !parsed.hostname.match(/^\d+\.\d+\.\d+\.\d+$/)) {
		return 'Invalid hostname: must be a fully qualified domain';
	}
