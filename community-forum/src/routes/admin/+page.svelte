<script lang="ts">
  ...
  import { ndk, connectRelay, connectRelayWithNip07, isConnected, reconnectRelay, ensureRelayConnected, publishEvent } from '$lib/nostr/relay';
  ...
  async function handleApproveChannelJoin(request: JoinRequest) {
    try {
      ...
      await publishEvent(addUserEvent);
...
<div class="container mx-auto p-4 max-w-6xl">
  ...
  {#if $authStore.isNip07 && !authStore.getPrivkey()}
    <div class="alert alert-warning mb-4">
      ...
      <span>NIP-07 login detected. Whitelist operations (user approval, cohort changes) require passkey login to sign NIP-98 auth tokens. Event publishing (approve/reject via relay) still works.</span>
    </div>
  {/if}
