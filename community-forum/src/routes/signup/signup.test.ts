 ...
 * 3. +page.svelte waits for profile indexing with exponential backoff before Kind 9024
 * 4. Whitelist status check before proceeding
 * 5. Error handling when publish fails
 *
 * Note: The 300ms delay was replaced with waitForProfileIndexing() which uses
 * exponential backoff (100ms → 200ms → 400ms → 800ms) with 5s max timeout.
 * See $lib/nostr/profile-sync.ts for implementation details.
 *
...
describe('Signup Flow Integration', () => {
  ...
  describe('Registration Request Publishing', () => {
    it('should publish registration request after profile indexing is confirmed', async () => {
      checkWhitelistStatus.mockResolvedValue({
        isApproved: false,
        isAdmin: false
      });
      ...
      if (!status.isApproved && !status.isAdmin) {
        // Simulate waitForProfileIndexing with exponential backoff
        // In production, this uses 100ms → 200ms → 400ms → 800ms with 5s max
        // Here we simulate the successful indexing confirmation
  ...
  describe('Profile Indexing Confirmation Behavior', () => {
    it('should use exponential backoff to confirm profile indexing before Kind 9024', async () => {
      ...
      // Simulate waitForProfileIndexing with exponential backoff
      // First attempt at 100ms, then 200ms, 400ms, 800ms up to 5s max
      // In this test we simulate quick success at first attempt
      await new Promise(resolve => setTimeout(resolve, 100));
      vi.advanceTimersByTime(100);
      ...
      // Should wait at least one backoff interval (100ms minimum)
      expect(timeDifference).toBeGreaterThanOrEqual(100);
    });
    ...
    it('should prevent race condition where registration arrives before profile', async () => {
      ...
      // waitForProfileIndexing confirms profile exists on relay before continuing
      // Using exponential backoff: 100ms → 200ms → 400ms → 800ms, max 5s
      await new Promise(resolve => setTimeout(resolve, 100));
      vi.advanceTimersByTime(100);

      // Only then publish Kind 9024 (after profile indexing confirmed)
    ...
    it('should handle exponential backoff timing correctly', async () => {
      // Simulate multiple attempts with exponential backoff
      const delays = [100, 200, 400, 800]; // Exponential backoff sequence
      let totalDelay = 0;
      ...
      // Total delay after 4 attempts would be 1500ms (100+200+400+800)
      expect(totalDelay).toBe(1500);
    });
  });
  ...
  describe('Complete Signup Flow Integration', () => {
    it('should complete full signup flow for new user', async () => {
      ...
      if (status.isApproved || status.isAdmin) {
        ...
        // Wait for profile indexing with exponential backoff before publishing registration
        // In production: waitForProfileIndexing() uses 100ms → 200ms → 400ms → 800ms backoff
        await new Promise(resolve => setTimeout(resolve, 100));
        vi.advanceTimersByTime(100);
