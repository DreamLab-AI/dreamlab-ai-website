 ...
 * @returns Always false - NIP-04 is removed for new messages
 */
export function isNip04EncryptionAllowed(): boolean {
  return false; // NIP-04 removed as of 2025-12-01 - use NIP-44 for new messages
}

/**
 * NIP-04 decryption is allowed READ-ONLY for historical messages
 * This prevents data loss for users with legacy DMs encrypted via NIP-04.
 * New messages must use NIP-44, but old messages can still be read.
 * @returns true - legacy read-only decryption is allowed
 */
export function isNip04DecryptionAllowed(): boolean {
  return true; // Allow read-only decryption of historical NIP-04 messages
}
