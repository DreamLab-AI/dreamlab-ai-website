...
//! WebID profile for a DreamLab user identified by their Nostr pubkey.
...
pub fn generate_webid_html(pubkey: &str, name: Option<&str>, pod_base: &str) -> String {
    let display_name = name.unwrap_or("DreamLab User");
...
<head>
  <meta charset="utf-8">
  <title>{display_name} - DreamLab</title>
...
mod tests {
    ...
    fn webid_contains_pubkey() {
        let html = generate_webid_html("abc123", None, "https://pods.dreamlab-ai.com");
        assert!(html.contains("abc123"));
        assert!(html.contains("did:nostr:abc123"));
        assert!(html.contains("DreamLab User"));
    }

    #[test]
    fn webid_uses_custom_name() {
        let html = generate_webid_html("abc123", Some("Alice"), "https://pods.dreamlab-ai.com");
